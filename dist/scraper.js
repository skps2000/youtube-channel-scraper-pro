            ...
            async requestHandler({ request, page, crawler: currentCrawler }) {
                ...
                if (type === 'VIDEO_PAGE') {
                    ...
                    try {
                        ...
                        await page.waitForSelector('like-button-view-model button, ytd-toggle-button-renderer button', { timeout: 20000 }).catch(() => null);
                        const likeText = await page.evaluate(() => {
                            ...
                            if (legacy)
                                return legacy.getAttribute('aria-label') || legacy.textContent || '';
                            return '';
                        });
                    ...
                    if (options.minLikes && (videoData.likeCount ?? 0) < options.minLikes)
                        return;
                ...
                try {
                    ...
                    channelName = await page.evaluate(() => {
                        const el = document.querySelector('yt-page-header-view-model h1 span') ||
                ...
                for (const card of cards) {
                    ...
                    const normalizeDate = (d) => d.replace(/(\d+)d ago/i, '$1 days ago')
